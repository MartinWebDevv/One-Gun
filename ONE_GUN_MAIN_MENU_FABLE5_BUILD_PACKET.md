# ONE GUN Main Menu — Fable 5 Build Packet
...
Give Fable 5 the reference image and this file. Send the prompts below **one at a time and in order**. Review each completed section in Godot before sending the next prompt.
...
These are the non-negotiable rules Fable should follow through the entire build.
...
Start by reporting the assets and systems you found, the files you expect to create or edit, and any genuine blockers. Then implement only the section I provide.
```
...
stop at after each section. let me test to see each section before continuing to the next section.