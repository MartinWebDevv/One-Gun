...
func _gate_entered(body: Node3D, index: int) -> void:
	...
	if index == 0:
		if not runners.has(id):
			state_dirty = true
			runners[id] = {"start":Time.get_ticks_msec(),"penalty":0,"next":1,"falls":0,"rules":movement_key(),"assisted":_assisted(body),"recovery":Space.RECOVERY[0]}
		return
	...
	if index == Space.COURSE_GATES.size()-1:
		...
		var bucket: String = run.rules+("/assisted" if bool(run.assisted) else "/standard")
		records.submit_completed_run(bucket,str(id),time_ms)
		runners.erase(id)
...
func _physics_process(delta: float) -> void:
	...
	if NetworkManager.is_host():
		for id in runners.keys():
			...
			if not Space.in_course(p) and not (p.x < -7 and p.z < -67.5 and p.z > -74):
				...
				state_dirty = true
				continue
			runners[id].assisted = bool(runners[id].assisted) or _assisted(actor)
		...
		if sync_left <= 0:
			sync_left = 0.1
			if not runners.is_empty() or state_dirty: _publish()
	...
	if running: hud.text += "\nCHECKPOINT %d / 5 / FALLS %d" % [int(run.next)-1,int(run.falls)]

func _snapshot() -> Dictionary:
	var times: Dictionary = {}
	for id in runners: times[id] = Time.get_ticks_msec()-int(runners[id].start)+int(runners[id].penalty)
	return {"indices":range_indices,"hits":hits,"runners":runners,"times":times,"records":records.lobby_times}
...
func _receive(data: Dictionary) -> void:
	var records_changed: bool = records.lobby_times!=data.records
	for lane in range(3):
...
func cancel_all() -> void:
	if NetworkManager.is_host(): runners.clear(); _publish()
