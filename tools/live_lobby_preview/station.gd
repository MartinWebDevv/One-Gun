...
func _ready() -> void:
	...
	_build_player_hub()
	_build_playpen()
	preload("res://tools/live_lobby_preview/scrap_space.gd").build(self)
	_build_arrivals()
	_build_departure()
	_build_lighting()
...
func _build_shell() -> void:
	...
	preload("res://tools/live_lobby_preview/bounded_concourse.gd").build(root_geo,G.material("concourse_cream",Color("e7d4b6")))
...
func _build_event_board() -> void:
	preload("res://tools/live_lobby_preview/dispatch.gd").build(self)

func _build_playpen() -> void:
	preload("res://tools/live_lobby_preview/playpen_space.gd").build(self)

func _build_arrivals() -> void:
	preload("res://tools/live_lobby_preview/arrival_space.gd").build(self)
...
func _zone(id: String, pos: Vector3, size: Vector3) -> void:
	...
	area.body_entered.connect(func(body):
		if body==get_parent().get_node_or_null("Player"): station_entered.emit(id))
	area.body_exited.connect(func(body):
		if body==get_parent().get_node_or_null("Player"): station_exited.emit(id))
...
func _build_player_hub() -> void:
	preload("res://tools/live_lobby_preview/arcade.gd").new().build(self)
