extends Node3D
const OUT := "res://artifacts/quality_pass_20260907/profile"
var rows: Array = []
var _recent: Array = []
var _pipeline: Dictionary = {}
func _added(node: Node) -> void:
	var script = node.get_script()
	if script != null or node is GPUParticles3D:
		_recent.append({"ms":Time.get_ticks_msec(),"name":str(node.name),"script":script.resource_path if script != null else "particles"})
		if _recent.size() > 20: _recent.pop_front()
func _pipelines() -> Dictionary:
	var values := {}
	for key in _pipeline: values[key] = RenderingServer.get_rendering_info(_pipeline[key])
	return values
...
func _run() -> void:
	...
	for key in ClassDB.class_get_integer_constant_list("RenderingServer"):
		if key.begins_with("RENDERING_INFO_PIPELINE_COMPILATIONS_"): _pipeline[key] = ClassDB.class_get_integer_constant("RenderingServer", key)
	...
	for cycle in range(2):
		for index in MapRegistry.map_count():
			...
			var previous_pipelines := _pipelines()
			var spikes: Array = []
			...
			while Time.get_ticks_usec()<end:
				...
				if now-previous > 33000:
					spikes.append({"frame_ms":(now-previous)/1000.0,"process_ms":Performance.get_monitor(Performance.TIME_PROCESS)*1000.0,"physics_ms":Performance.get_monitor(Performance.TIME_PHYSICS_PROCESS)*1000.0,"recent":_recent.duplicate(),"pipelines":_pipelines(),"pipelines_at_sample_start":previous_pipelines})
				previous=now
			times.sort()
