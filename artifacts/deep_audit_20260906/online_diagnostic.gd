...
func _run_host_checks() -> void:
	...
	if gun.is_held or melee.is_held or not gun.can_fire or victim == null or victim.is_eliminated:
		print("AUDIT reset state gun-held=",gun.is_held," melee-held=",melee.is_held," fire=",gun.can_fire," victim=",victim," eliminated=",victim.is_eliminated if victim!=null else null)
	...
	while Time.get_ticks_msec() < deadline:
		...
		if local_player != null and scene != null and scene.scene_file_path == test_map:
			...
			if not local_player.is_eliminated and hud.player != local_player:
				print("AUDIT transient HUD binding; checking after two frames")
				await get_tree().process_frame
				await get_tree().process_frame
				if hud.player == local_player:
					print("AUDIT HUD recovered within two frames")
					continue
