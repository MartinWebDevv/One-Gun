...
**Bots exist to fill an asymmetric local multiplayer gap.** Because splitscreen only supports 2 human players but the match supports up to 8 combatants, bots (with 4 difficulty tiers) are the mechanism for scaling a 1–2 person session into a full arena match. Their behavior mirrors human capabilities such as dash and retreat rather than being a separate simplified system.
