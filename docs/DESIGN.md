...
**Scarcity creates drama, supply keeps everyone active.** The gun remains singular. Every authored melee marker begins populated and independently replenishes 5s after pickup; every item and powerup marker is likewise populated and replenishes on an 8s pickup/collection cycle. Random melee type/effect rolls keep those counterplay tools eventful without diluting the one-gun objective.

**Melee is a disarm tool, not a kill tool, by default.** `melee_eliminates_gunholder` and `melee_eliminates_anyone` both default to `false` in `game_config.gd`. The base design intends melee to threaten the gun holder's *possession* of the gun, not their life — killing with melee is an optional house rule, not the default loop. Retiring weapon tiers and sharing one hit volume keeps success about positioning and timing rather than a lucky range or tier roll.
...
**Bullet immunity windows exist so tension doesn't collapse instantly.** After a stagger or knockback, the victim gets a fixed brief window of bullet immunity. This is transient combat protection rather than an unearned Sticky Hands or Extra Life charge. Without it, "get staggered → get shot" would be an unavoidable death loop; the immunity window preserves a chance to scramble away.
...
**Bots keep small groups active.** Because splitscreen only supports 2 human players but the match supports up to 10 combatants, bots (with 4 difficulty tiers) are the mechanism for scaling a 1–2 person session into a full arena match. Bots also fill online host-configured slots. They use movement, melee, item and powerup systems while the host owns online AI; difficulty scales decisions rather than granting another damage model.
...
## 5. Current scope and open decisions

- **Online play is implemented:** ENet listen hosts, named/unlisted discovery, direct endpoints, dedicated hosting, ticket admission and coordinated loading are live code paths. Local play remains supported. Reconnect and host-migration policy still need a deliberate player-facing design.
- **Cosmetics have a pipeline:** stable registry IDs, per-model Hat fits, shared collision, transactional preview/confirm and lazy animation loading. New garment art and its production rules remain future work.
- **Bots use items and powerups:** maintain human/bot rule parity and validate authority on dedicated servers when changing combat behavior.
- **Combat already has audio feedback:** shots, melee, disarms, hit confirmation and confetti eliminations have sound paths. Future audio decisions concern tuning and identity, not a missing combat audio system.
- **Unlisted is a discovery choice:** anyone with a reachable endpoint or share code can join; it is not enforced Friends Only or invitation admission.
- **Hideout is the home and shared pre-match space:** the authorized production entry uses real menu destinations and host-owned practice activities; launching starts a match together and returning preserves the session. The original F6 rehearsal remains isolated. Atomic Squad travel, authenticated privacy and world records are subsequent systems; see [HIDEOUT_MIGRATION.md](HIDEOUT_MIGRATION.md).
