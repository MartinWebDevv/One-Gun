...
> 2026-09-08: current source uses network protocol **7**. Re-export matching clients and dedicated servers, then update/deploy the coordinator with `NETWORK_PROTOCOL=7` as one release. Older examples and successful live gates below describe previous deployments. These source changes do not publish a server or Worker.
