# ONE GUN Character, Weapon, Animation & Customization — Codex Ultra Production Packet
...
Codex works directly inside the existing ONE GUN repository. These rules supplement every section and hard gate in this packet:

- Read all applicable repository instructions, including AGENTS.md and existing project documentation, before editing.
- Inspect git status first and preserve unrelated or pre-existing user changes.
- Inspect existing Godot, Blender, character, weapon, animation, networking, save, and customization systems before creating replacements.
- Implement the requested section when possible; do not stop at a plan unless a genuine blocker prevents implementation.
- Make only the changes authorized by the current unlocked section.
- Use apply_patch or other safe repository editing methods consistent with the active environment.
- Keep Blender source files and reproducible Blender Python helpers alongside the intended project asset structure.
- Use Blender Python for repeatable setup, validation, export, and controlled modeling assistance—not as an excuse to ship crude primitive assemblies.
- Run the safest relevant Godot parsing, import, project startup, automated checks, Blender validation, and export checks available after each section.
- Visually inspect fresh Blender renders and Godot captures whenever appearance changes. Never claim a close match without comparing against the approved concept.
- If a GUI or visual capture is unavailable, state that limitation clearly and do not claim visual approval.
- Report exact files created or edited, verification performed, results, remaining mismatches, and blockers.
- Do not commit, push, delete, reset, or rewrite unrelated files unless the user explicitly asks.
- Stop at every review point and hard gate. Codex cannot approve its own work.

## Master Production Map

Codex must follow this order. A later phase is locked until the user explicitly approves the preceding hard gate.
...
The approved Option C hydro ray pistol concept is the canonical base weapon design. Place the approved concept image in the Codex design packet under the clear filename `ONE_GUN_DEFAULT_HYDRO_RAY_CONCEPT.png`.
...
- If Codex is started in a new conversation, it must inspect the project and identify the last explicitly approved gate before editing.
- Codex must execute only the next allowed section, provide its required review evidence, and stop.
- Codex must never skip ahead because a later task appears technically possible.
...
Give Codex access to the ONE GUN repository, this packet, and the approved concept sheets, then say:

> Read the complete ultra packet and all applicable repository instructions. Inspect the current project and git status, determine the current approved production stage, execute only the next allowed section, validate the result, and stop at its required review point.
...
- Direct access to the existing ONE GUN repository/project folder
- The supplied mascot concept sheet
- The approved `ONE_GUN_DEFAULT_HYDRO_RAY_CONCEPT.png` weapon sheet
- This Markdown packet

Send the prompts below **one section at a time and in order**. Codex must stop after every section and provide the requested renders, screenshots, files, and validation results for approval.

Do not allow Codex to create the gun and all eight mascots at once. The **Blue Cat must complete the entire production pipeline and receive approval in Blender and Godot before another mascot begins**.

Near-1:1 means matching the approved reference's silhouette, proportions, facial construction, colors, rounded finish, personality, and visual weight. Where the single concept sheet does not show a side or back design, Codex must not silently invent a final design; it must create a reviewable turnaround proposal and wait for approval.
...
Before implementing the current section, inspect all relevant existing files, repository instructions, and git status; report genuine blockers. Implement when possible rather than stopping at a plan. Validate the work, report exact file changes, and stop after the requested section.
```
...
Codex reaches this gate only after Sections 2–7 are complete.
...
Codex must stop here. It may not begin the shared skeleton, customization implementation, weapon grips, or animation work.
...
Stop at Hard Gate B. Codex cannot approve its own gun and cannot resume mascot Section 8 until the user explicitly approves the final weapon.
~~~
...
- Codex resumes at mascot **Section 8 — Player Customization Architecture**.
