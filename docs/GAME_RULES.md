...
> Source of truth: this document describes mechanics as implemented in the codebase as of 2026-08-27. Numbers are pulled directly from gameplay, configuration, registry, round, reward, database, weapon, item, and bot code. Where a value is configurable per match, the default is listed.
...
- Reaching `sets_per_match` (default **1**) wins the **match**. Free-for-all One Gun/All Gun matches enter the Winners Circle before returning to the lobby; team and One of Us endings retain the legacy direct return for now.
- Display timings: countdown before round start **3s**, round-end banner **3s**, set-end banner **4s**, and legacy match-end banner **6s** (all configurable on `RoundManager`). FFA match winners hold their victory move for a **2.75s** handoff, then crossfade into the Winners Circle instead of waiting for that legacy banner.
- Before the countdown of the **first round only**, each player gets a frozen 3s camera introduction: a map-authored `round_intro_camera_point` begins above the arena, normally orbits 180° toward that player's spawn, then fades into their gameplay camera. A map may author a narrower synchronized sweep when a full orbit would leave its playable perimeter. Reduced Motion replaces the orbit with a simple fade.
...
- A round defaults to a host-adjustable **3:00** limit (`round_time_limit`). It can be adjusted in one-second increments down to **1s**; **0** disables the timer. When it expires with multiple survivors, the round enters unlimited-count-up **Overtime** instead of ending.
...
- Standard OT preserves the one-gun contest. Enabling `chaos_overtime_enabled` arms every survivor and retires the original shared gun. Both modes preserve melee weapons already held at the transition; Chaos clears carried items/powerups but not held melee. All regular pickup placements close, then exactly one randomized melee supply remains active at the exact center of the map on its independent 5s refill cycle. Melee weapons already dropped into the world remain valid pickups throughout OT. OT time shown in the HUD uses minutes/seconds; milliseconds remain internal for simultaneous-elimination tiebreaks.
...
The Level Road grants Gun Tokens at levels **5/15/25/35/45/55/65/75/85/95** for **50/75/75/100/100/125/125/150/150/200** tokens. The Beta Season **Rice Hat** now unlocks at **Level 10**; the old Level 30 placement is retired. Other permanent seasonal cosmetics continue at their authored milestones. After level 100, every tenth level grants **100 Gun Tokens** indefinitely.

The Trophy Road grants permanent seasonal cosmetics at authored Official Classic victory milestones. The Beta Season **Pimp Hat** now unlocks with **5 Trophies**; its old Trophy 20 placement is retired. Road prizes unlock once, enter the owned-only Locker permanently, and never disappear after a season reset. Any cosmetic milestone on either road is selectable with mouse, keyboard, or controller and opens a small preview of the player's currently selected character wearing that exact reward; token milestones remain informational.
...
The normal-motion ceremony is a shared approximately 10s reveal: it fades in while pulling away from third place, swishes to second, then follows a large orbit around the champion before settling into the first-place hero shot. All three selected podium celebrations are already running from the first visible frame. Every owned animated dance may be assigned independently to the Winners Circle slot, the Round Win slot, or both. The Trophy drops and confetti fires at the champion beat, then the screen briefly fades through black into the normal Winners Circle layout. Shared standings, personalized results, Full Stats, and Ready controls remain hidden for the cinematic. Afterward, results remain open indefinitely until every connected participant presses Ready or the listen host/dedicated-session lobby moderator uses Return All Now; only the synchronized return itself shows a short countdown. The champion's equipped Winners Circle theme plays for all peers; Ceremony March is the free/default fallback. Reduced Motion uses a short static wide reveal, long idles, immediate Trophy placement, no confetti, and the same results interface afterward.

An Official Classic winner also receives a trophy-drop presentation in front of the champion, independent of their skeleton and dance. The built-in placeholder is used until `res://models/rewards/winners_circle_trophy.glb` exists. The personalized result panel animates the persisted Season XP gain through a current/next-level progress bar, then shows Trophy progress when applicable or Gun Tokens when no Trophy was earned, and finally reveals the current-match reward box with road bonuses and unlocks from Supabase.

Every participant sees shared final standings plus a personalized results card. Online participants may mark themselves Ready after a **10s minimum viewing time**; when every still-connected participant is ready, everyone sees a synchronized **3s** return countdown. The listen host, or the human lobby moderator when peer 1 is dedicated, may return everyone after the same minimum. The results modal owns pointer/controller input while open so background spectator cycling cannot consume those actions. The results remain open until one of those explicit conditions is met; there is no automatic results timeout. Local players receive separate Ready controls and results cards, but local matches are always results-only.
...
**Aiming**: Non-ADS keeps the established 4.0m third-person boom and full configured FOV, but uses a distance-compensated wide shoulder offset of `(1.02, 0.23, 0.06)` plus a `0.03m` camera lift so every shipped Hat remains left of the center sight lane. ADS (aim-down-sights) transitions over 0.2s, tightens FOV to 0.86×, and pulls the spring arm to 2.40m while smoothly settling into the approved closer shoulder offset `(0.85, 0.38, 0.10)` plus a final `0.05m` camera lift. ADS halves look sensitivity by default (configurable, `ads_sensitivity_multiplier`).
...
|---|---|---|---|---|---|---|
| Sword | 5.2m | 0.32s | 0.28s | 0.55s | 15 | shared Frying Pan timing |
| Baseball Bat | 5.2m | 0.32s | 0.28s | 0.55s | 15 | shared Frying Pan timing |
| Stick | 5.2m | 0.32s | 0.28s | 0.55s | 8 | shared timing, cheapest stamina |
| Crowbar | 5.2m | 0.32s | 0.28s | 0.55s | 14 | shared Frying Pan timing |
| Frying Pan | 5.2m | 0.32s | 0.28s | 0.55s | 22 | shared timing, highest stamina |
...
Windup (wind-up pose, no hit) → Active (hitbox live) → Recovery (character returns to locomotion). The authored character melee animation owns the visible attack; held weapon objects remain fixed to their paw socket instead of adding a second, smaller rotation tween. All five weapons use the Frying Pan's authored timing. Runtime swing phases use 85% of those data-authored durations: 0.272s windup, 0.238s active, and 0.4675s recovery. Fast Hands divides all three runtime phases by 1.5 while active; it does not change stamina cost. Stamina is paid **upfront** at swing start. Swinging while in stamina deficit twice in a row **breaks the weapon** for 5.0s (it respawns at its spawn point) — this can be disabled via `melee_weapon_breaking`.
...
| Bear Trap | hazard | Deploys where it lands at 130% of its former visual and collision size, sits open up to 20s; first player in gets rooted 1.5s (stagger), trap snaps shut. One use. Owner-safe per `can_affect`. |
| Decoy | consumable | Deploys one fake player for up to 10s. It runs straight forward by default; the owner can press **C** or D-pad Up to toggle mirrored movement control. Bots target it like a real player, while attacks and hostile traps affect or pop it. The owner and protected teammates cannot destroy it. An enemy gunshot pop briefly outlines the shooter for 0.5s; trap-triggered pops do not. It awards no kill credit. |
| Boomerang | consumable | Flies ~10m out and returns over ~1.7s; disarms a gun holder on hit (respects melee shields, grants the victim 1s immunity), knocks back anyone else. A missed first throw is auto-caught if a slot is free and grants one second throw. Any hit or the second missed return consumes it. |
...
Powerups spawn at map markers as bobbing, camera-facing pixel-art icons. The transparent world-space sprite and its name label billboard independently for every camera, so local splitscreen players and spectators each see the icon front-on. Its type is **fixed at spawn** and changes only when that powerup respawns after collection.

| Powerup | Effect |
|---|---|
| Extra Dash | +1 dash charge for 5.0s |
| Sticky Hands | Blocks the next gun disarm attempt for up to 30s. It cannot stack or be collected while active. Expiration or use starts an 8s recollection cooldown. |
...
| Fast Hands (pink) | for 5s, increases melee swing speed by 50%; windup, active, and recovery are divided by 1.5 while stamina usage is unchanged. Its world pickup uses `fastHands2D.png`. | yes |

Powerup type is **fixed at spawn** and re-rolls only when the pickup respawns after being collected. Items and powerups spawn at map-placed `item_spawn_point` / `powerup_spawn_point` markers each round. Every item marker rolls a random enabled type (pool = `GameConfig.ITEM_SCENES` filtered by `is_item_enabled`) and starts an independent **8.0s refill when that item is picked up**; the carried object remains usable, then retires after use instead of creating a second refill. Powerups likewise re-roll after their 8.0s post-collection delay. Marker-spawned nodes are tracked via the `marker_spawned` group and freed/re-spawned each round reset.

`powerups_enabled` is the collectible-powerup master gate, and `powerup_registry` controls Extra Dash, Sticky Hands, Speed Surge, Silent Steps, Extra Life, Reach, and Fast Hands individually. An empty enabled powerup pool creates no powerup placements; this is allowed, unlike the melee pool. Mode rules filter the enabled registry at spawn time: All Gun excludes Extra Life and Fast Hands, while One of Us excludes Extra Life and Sticky Hands; only Them may collect Fast Hands in One of Us.

Online powerup collection is host-validated and applied on every peer. Persistent defensive state is resolved by the host: Extra Life prevents an authoritative lethal blow before scores/deaths change, and Sticky Hands blocks one gun-disarm attempt for at most 30s before starting its 8s recollection cooldown. The protections can be held together but cannot stack with themselves, and current protection, Sticky Hands duration/cooldown, and immunity state are included in late-spectator synchronization. Temporary immunity caused by a knockback/stagger melee effect remains separate and expires authoritatively even when the affected actor belongs to a client. During follow spectating, the complete inventory, stamina, dash, active-effect, reload/cook, and throw-preview HUD is rebound read-only to the followed actor.
...
Two independent input sets exist for splitscreen (`p1_*` / `p2_*` action prefixes). The persistent **Player Settings → Controls → Primary Player** selector defaults to **Mouse & Keyboard** and can be changed manually to **Controller**. Player 2 uses a separate controller in splitscreen. When P1 also selects Controller, the first detected pad routes to P1 and the second to P2; hot-plugging refreshes that routing. Keyboard/mouse and controller are both fully supported, including right-stick look with independent horizontal and vertical sensitivity sliders (both default to 6.0), a configurable radial response curve, a 0.15 deadzone, and remappable primary/secondary binding slots. Every title, Player Hub, social, lobby, settings, Locker, Prize Counter, and progression control has an explicit gamepad focus path.

Core actions: move (WASD / left stick), look (mouse / right stick), jump (Space / A/Cross), dash (Ctrl / B/Circle), Fire/use (LMB / RT/R2 analog: gun, melee swing, or active item), ADS/cancel camera (RMB / LT/L2 analog), interact/pickup/drop (F by default / X/Square — tap vs. hold, see §9), melee throw preview/release (G / Y/Triangle), cycle slots (Q/E or LB/L1 and RB/R1), and decoy-control toggle (C / D-pad Up). Left-stick click is the default controller Sprint binding; Sprint remains configurable but does nothing while the global sprint gate is disabled. Start/Menu pauses, Back/View holds the scoreboard, A/Cross selects UI controls, and B/Circle goes back while a menu is open.

Spectating uses LMB/RMB (LB/RB) to cycle living players and Space (A/Cross) to switch between follow and free cameras. Free camera uses WASD/left stick to fly, mouse/right stick to look, Shift/RT to rise, Ctrl/LT to descend, and **F** or left-stick click to move fast.
...
Human players may choose the Male or Female V2 model and any of 13 color textures: Black, Blue, Brown, Cyan, Green, Grey, Orange, Pink, Purple, Red, Salmon, White, or Yellow. Both models use the Male V2 idle as the shared master idle while retaining their other model-specific animation mapping. M and F buttons beneath the still 3D preview switch the pending model immediately; colors can still be cycled or selected from the 5/5/3 portrait grid. The preview remains still until the player drags with the left mouse button or moves that player's right stick. The main title rail contains Play, Player Hub, Settings, and Quit; its Player Hub groups Profile, Locker, Prize Counter, and Progression. The online-lobby Player Hub contains Locker, Prize Counter, and Progression. Friends instead opens from a circular blue-mascot portrait in the upper-right of the home screen and online lobby; hover/focus reveals the themed FRIENDS label. The orb is not instantiated during matches, the Winners Circle ceremony, or final results. The Locker allows duplicate appearances. Its Character / Weapons / Victory / Audio categories and nested clothing/outfit/skin/pose/dance/theme filters show only cosmetics owned by the authenticated Player 1 account; animated dances can be assigned to either independent celebration slot. Equipped cards read UNEQUIP. A two-click Reset Loadout clears cosmetic pieces and returns weapons, celebrations, and ceremony music to their base choices while preserving the selected model and color. Base character colors remain freely selectable, and local Player 2 remains a guest appearance. P1's model and color persist together in `PlayerPrefs`; local P2's pair is session-only. Online peers synchronize stable appearance/loadout IDs through the lobby and match spawn data. Bots retain their existing model and appearance. A deployed decoy copies its owner's selected human model and color.

Authenticated players may send, accept, or deny friend requests from the Friends panel. Friends can be viewed online from the home-screen and online-lobby mascot orb, and a friend may be invited to the current named Tailscale lobby. A received lobby invitation appears as a controller-focusable notification with Accept and Deny actions; accepting uses the existing named-lobby join path. Presence is a lightweight cloud heartbeat; a joinable lobby endpoint is visible only to accepted friends, never anonymous users or non-friends.

Twelve locally rendered Headwear cosmetics attach to a fur-contact seating plane derived from the animated `Head` socket on both V2 models. Rice Hat is the Beta Level 10 reward and Pimp Hat is the Trophy 5 reward. Chef Hat and Yellow Point Hat cost 850 Gun Tokens; Classic Cowboy Hat, Black Fedora, and Straw Adventurer Hat cost 1,250; Wide-Brim Cowboy Hat, White Fedora, Top Hat, and Witch Hat cost 1,650; Royal Crown costs 2,200. Those ten purchasable hats rotate through the Prize Counter. Prize Counter hat inspection uses an adaptive shoulder-up camera and remains still until mouse-dragged or moved with the right stick. Locker and progression previews remain full-body but recenter from the selected hat's actual top bound so it retains comfortable top clearance. Stable cosmetic IDs preserve ownership and equipped loadouts independently of source filenames.
...
## Quality and consistency pass — 2026-09-07
