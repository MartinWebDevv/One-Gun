...
- Display timings: countdown before round start **3s**, round-end banner **3s**, set-end banner **4s**, and legacy match-end banner **6s** (all configurable on `RoundManager`). FFA match winners enter the Winners Circle after a short **0.75s** handoff instead of waiting for that legacy banner.
...
FFA One Gun and All Gun match endings freeze a result snapshot and open a dedicated full-screen Winners Circle. First place is the match champion; remaining places sort by total round wins, runner-up finishes, third-place finishes, kills, disarms, then stable actor ID. The top three appear simultaneously on 2nd/1st/3rd podiums using their selected character appearance and equipped Victory Move; missing moves fall back to a long idle. The champion's ceremonial gun display uses the default One Gun model until a mapped equipped gun skin exists.
