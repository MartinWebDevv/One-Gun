...
Every participant sees shared final standings plus a personalized results card. Online participants may mark themselves Ready after a **10s minimum viewing time**; when every still-connected participant is ready, everyone sees a synchronized **3s** return countdown. The host may return everyone after the same minimum. The results remain open until one of those explicit conditions is met; there is no automatic results timeout. Local players receive separate Ready controls and results cards, but local matches are always results-only.
