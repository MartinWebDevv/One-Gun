...
- **Online multiplayer (Phases 1–2e, 2026-07-11 to 2026-07-13):** `NetworkManager` handles ENet/Tailscale sessions, named-lobby discovery, roster/name exchange, config sync and coordinated scene readiness. `round_manager.gd` runtime-strips the baked local setup, then spawns humans and host-configured bots through one `MultiplayerSpawner` without editing maps. Humans retain owner-authoritative movement; bots have stable actor IDs from 10000 upward, run AI only on peer 1, and replicate as client puppets. Gun, melee, item/powerup, elimination and scoring paths are host-resolved and actor-ID addressed. Key patterns to preserve: every combat request carries a round epoch; reparented melee/items use the stable `RoundManager` RPC path; `online_actor_state` is the scoring/HUD snapshot; match start waits for every peer's scene-ready signal; death lasts until round reset; the host refuses mid-match joins. Online teams remain disabled. All online code is gated behind `is_online()`; local play is untouched.
